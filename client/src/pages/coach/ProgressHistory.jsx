...
const ProgressHistoryCoach = () => {
  ...
  useEffect(() => {
    const loadProgress = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`/coach/progress?userId=${userId}`, {
  ...
  const handleSaveFeedback = async (schedule, comments) => {
    ...
    try {
      await axios.put(
        `/coach/progress/${selectedProgress.id}/feedback`,
        {
          schedule,
          comments,
        },
...
export default ProgressHistoryCoach;
